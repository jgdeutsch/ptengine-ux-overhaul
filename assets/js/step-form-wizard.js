/*!
 * Step Form Wizard
 * Jiøí Waœniowski - <ajoke3@gmail.com>
...
(function($) {
    ...
    var defaults = {
        ...
        showButtons: true,
        showLegend: true,
        nextBtn: $('<a class="next-btn sf-right sf-btn" href="#">NEXT</a></p>'),
    ...
    function stepFormWizard(element, options) {
        ...
        w.labels = [];
        w.themes = {sun: 't1', sea: 't2', sky: 't3', simple: 't4', circle: 't5'}
    ...
    stepFormWizard.prototype.init = function() {
        ...
        w.wizard.addClass('sf-' + w.themes[w.config.theme] + ' sf-' + w.config.transition );
        ...
        w.element.on('keydown', ':input:not(textarea)', function(e) {
    ...
    stepFormWizard.prototype.wrapStep = function(step, index) {
            ...
            var wrap_div = $('<div>', {class: 'sf-step sf-step-' + index}).data('step', index);
    ...
    stepFormWizard.prototype.addStep = function(index, body) {
        var w = this;
        ...
        $('.sf-nav-step', w.wizard).each(function() {
            ...
            if(stepNavNum >= index) {
                ...
                $(this).find('.sf-nav-number-inner').text(stepNavNum + 1);
            }
        })
    ...
    stepFormWizard.prototype.removeStep = function(index) {
        var w = this;
    ...
    stepFormWizard.prototype.navNumber = function(index, num) {
        this.navStep(index).find('.sf-nav-number').html(num);
    }
    ...
    stepFormWizard.prototype.wrapNavItem = function(step, index) {
        ...
        if(w.navStep(index + 1).length) {
            ...
            nav_li.show(300);
            setTimeout(function() {w.careNav(w.stepActive, w.stepActive);}, 300);
        } else {
    ...
    stepFormWizard.prototype.goTo = function(index, data, ifFinish) {
        ...
        w.element.find('.sf-step').removeClass('sf-step-no-active').addClass('sf-step-active');
        ...
        if(w.config.transition == '3d-cube') {
            $('.sf-fieldwrap', this.wizard).css({
                transform: 'translateZ(-'+(w.stepWidth - w.stepWidth/4) +'px)'
            });
            setTimeout(function() {
                $('.sf-fieldwrap', w.wizard).css({
                    transform: 'translateZ(-'+(w.stepWidth - w.stepWidth/4) +'px) rotateY('+stepShift+'90deg)'
                });
            }, w.config.duration)
            setTimeout(function() {
                $('.sf-fieldwrap', w.wizard).css({
                    transform: 'translateZ(-'+w.stepWidth/2+'px) rotateY('+stepShift+'90deg)'
                });
            }, w.config.duration * 2)
            setTimeout(function() {
            ...
            $('.sf-fieldwrap', this.wizard).css({
                transform: 'translateX(' + stepShift+(w.stepWidth) + 'px)'
            });
            setTimeout(function() {
                w.afterTransition(w.stepActive, index, data);
            }, w.config.duration)
                ...
                if(navStepsWidth > navWidth) { // nav must be more width than page
                    ...
                    if(next_step.length) {
                        $('.sf-nav', w.navWrap).css({
                            transform: 'translateX(' + sign + (actStepLeft + navOffset) + 'px)'
                        })
                        ...
                        if(navNextIndex < 0) { // first step
                            ...
                            $('.sf-nav', w.navWrap).css({
                                transform: 'translateX(' + sign + (actStepLeft) + 'px)'
                            })
                        }
                    }
                }
